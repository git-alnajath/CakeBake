        ...
        <div className="overall">
            <div className="navbar">
                <div className="nav-left">
                    <h3><span>TASTY</span> DESSERTS</h3>
                </div>

                <input placeholder={"Search here"} className={inp == true ? "int1" : "int"} onChange={stor} />

                <div className="nav-right">
                    <h2 onClick={inpt}><FaSearch /></h2>
                    <h2 onClick={detail}><CiViewTable />{count > 0 ? <sup>{count}</sup> : null}</h2>
                </div>
            </div>
            <div className={val == true ? "details2" : "details"}>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><b>{it && it.nam}</b></td>
                            <td><b>{howmany.filter((item) => item==it.id).length}</b></td>
                            <td><b>{it && it.rs}</b></td>
                        </tr>
                        )
                        }
            ...
            <div className="box">
                {arr.map((item) => (
                    (item.name.toLocaleLowerCase().includes(data2) ? <Project id={item.id} nam={item.name} picname={item.photo} rs={item.price} number={inc} /> : null)
                ))}
            </div>
        </div>
    )
